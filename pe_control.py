...
class PEControl(Elaboratable):
    def __init__(
        ...
        self.load_select = Signal(LOAD_DEST)
    ...
    def elaborate(self, platform):
        ...
        with m.FSM(reset='INIT'):
            with m.State('INIT'):
                m.d.sync += [
                    self.addr_io.eq(0),
                    self.magic_cnt.eq(self.magic_init+1),
                ]

                with m.If(self.in_r_data == self.magic_init):
                    m.d.sync += self.next_pc.eq(1)
                    m.next = 'FETCH'
            ...
            with m.State('DECODE'):
                with m.Switch(self.opcode):
                    ...
                    with m.Case(OPCODE.SET_M):
                        ...
                        # 4b     4b               24b
                        # set_m                   fan_in
                        m.d.sync += self.fan_in.eq(self.v2_f)
                        m.next = 'FETCH'
            ...
            with m.State('LOAD'):
                ...
                with m.If(self.load_select == LOAD_DEST.A):
                    m.d.sync += pipe_a.w_en.eq(1)
                with m.Else():
                    m.d.sync += pipe_b.w_en.eq(1)
